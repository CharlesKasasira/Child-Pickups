...
import cv2
import numpy as np
from .models import Parent, ParentsLog
...
PARENT_CASCADE_PATH = os.path.join(BASE_DIR, 'children_management', 'static', 'children_management', 'haarcascade_frontalface_default.xml')
PARENT_DATASETFACE_PATH = os.path.join(BASE_DIR, 'children_management', 'static', 'children_management', 'parents', 'datasetface')
PARENT_RECOGNIZER_PATH = os.path.join(BASE_DIR, 'children_management', 'static', 'children_management', 'parents', 'recognizer', 'trainingdata.yml')

face_cascade = cv2.CascadeClassifier(PARENT_CASCADE_PATH)


def base(request):
    return render(request, 'children_management/register_parent.html')
...
def handle_face_image(face_data_uri, parent_id):
    if face_data_uri:
        _, encoded_data = face_data_uri.split(',', 1)
        decoded_data = base64.b64decode(encoded_data)
        image = Image.open(BytesIO(decoded_data))
        image_path = f'parent_faces/{parent_id}.png'
...
def register_parent(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            parent.save()  # Save the parent instance first to generate an ID

            face_data_uri = request.POST.get('face_image')
            image_path = handle_face_image(face_data_uri, parent.id)
            if image_path:
                parent.face_image = image_path

            parent.save()
            return redirect(reverse('capture_and_train_parent', args=[parent.id]))  # Redirect to capture_and_train_parent with parent_id
    else:
        form = ParentForm()

    return render(request, 'children_management/register_parent.html', {'form': form})


@login_required(login_url='login')
def capture_and_train(request, parent_id):
    ...
    while True:
        ...
        for (x, y, w, h) in faces:
            ...
            cv2.imwrite(os.path.join(PARENT_DATASETFACE_PATH, f"user_{parent_id}_{sample_num}.jpg"), face_img)
...
def train_recognizer():
    ...
    faces, ids = get_images_with_id(PARENT_DATASETFACE_PATH)
    ...
    if not os.path.exists(os.path.dirname(PARENT_RECOGNIZER_PATH)):
        os.makedirs(os.path.dirname(PARENT_RECOGNIZER_PATH))
    recognizer.save(PARENT_RECOGNIZER_PATH)
    cv2.destroyAllWindows()
...
def recognize_faces():
    recognizer = cv2.face.LBPHFaceRecognizer_create()
    recognizer.read(PARENT_RECOGNIZER_PATH)
    cap = cv2.VideoCapture(0)
    ...
    while True:
        ...
        for (x, y, w, h) in faces:
            cv2.rectangle(img, (x, y), (x + w, y + h), (0, 255, 0), 2)
            parent_id, conf = recognizer.predict(gray[y:y + h, x:x + w])
            if conf < confidence_threshold:
                profile = get_profile(parent_id)
                if profile:
                    if ParentsLog.log_recognition(profile):
                        ...
                        log = ParentsLog.objects.filter(parent=profile).first()
                        if log:
...
def parents_logs(request):
    recent_logs = ParentsLog.objects.filter(recognition_time__gte=timezone.now() - timedelta(minutes=1000)).order_by('-recognition_time')
    ...
    return render(request, 'children_management/parents_logs.html', {'recent_logs': recent_logs})

def update_pickup_status(request, log_id):
    if request.method == 'POST':
        try:
            log = ParentsLog.objects.get(id=log_id)
        ...
        except ParentsLog.DoesNotExist:
